...
export class AwsSagemakerStack extends cdk.Stack {
...
//grep  -v '?' /tmp/jj | sort > /tmp/sage | grep '?' /tmp/jj | sort >> /tmp/sage; cat /tmp/sage | sed "/^}/d; /^export/d; s/;$/,/; s/readonly //; s/string/''/"      
    authMode: '',
    defaultUserSettings: UserSettingsProperty({
      
    }),
      executionRole: 'bluettrole',
